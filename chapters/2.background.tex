...
The automatic generation from event data arises important questions regarding their quality, reliability, and practical usefulness. The assessment of simulation quality, of directly from event logs derived model structures and parameters, has evolved into an active research area, sitting between traditional Process Mining and Business Process Simulation. Ultimately, the objective of a simulation model is not merely to reproduce historical data, but to support operational decision making. Therefore, simulation quality must be evaluated from multiple perspectives, including behavioral realism, temporal accuracy, resource representation, and decision-support capability.

\subsection{Dimensions of Simulation Quality} 
    Simulation models are multi-dimensional representations of real-world processes, therefore simulation quality cannot be reduced to a single performance indicator. Instead, different aspects of process behavior have to be evaluated separately.
    
    \paragraph{Control-Flow Quality} 
    The control-flow perspective describes if the simulation model reproduces the observed behavioral structure from the Event Log. A high-quality model should generate process variants and activity sequences that resemble the execution behavior recorded in the event log. Deviations in the models control-flow indicate e.g. missing process paths, unrealistic routing behavior or overly generalized models, allowing plausible but unobserved process paths \cite{PMRozinat2009,Chapela2025SimQuality}. 
    \paragraph{Temporal Quality} 
    Temporal quality of a simulation model measures how accurately the simulation reproduces timing-related process characteristics like activity durations, waiting times, throughput times and overall case durations. Decision-support applications, as a common goal of simulation models often focus on performance improvement, temporal realism is an important component of a simulations quality, that has to be critically assessed before implementation \cite{BPSAalst2015,Chapela2025SimQuality}. 
    \paragraph{Resource Quality} 
    Resource quality evaluates how accurately simulation models represent resource behavior. That includes resource assignments, individual workloads, utilization patterns, and learned capacity constraints. Resource related inaccuracies of a model can heavily affect simulation outcomes because waiting times and bottlenecks are often produced by resource contention rather than control-flow decisions alone \cite{BPSAalst2015,Vinci2026prosit}. 
    \paragraph{Decision-Support Capability} 
    The Decision-Support Quality of a Simulation model defines the its ability to remain sufficiently realistic under changed conditions and alternative scenarios defined by the developers. That also includes the ability of a model to support unobserved what-if analyses and generate trustworthy recommendations for operational decision making \cite{Chapela2025SimQuality,runtimeIntegration2024}.
...
\begin{itemize} 
\item the robustness of automatically discovered simulation models, 
\item their trustworthiness for operational decision support, 
\item their ability to reproduce congestion effects and workload dynamics, 
\item the practical value of white-box simulation in complex logistics environments. \end{itemize} 
