...
simulation is discovered and the deterministic pipeline that turns raw
operational records into a simulation-ready event log. The chapter is
descriptive by design: no models are fitted, no hypotheses are tested,
and no experimental parameters are chosen. Its role is to establish the
data foundation on which every subsequent claim in
Chapters~\ref{chp:experiments_and_framework}
and~\ref{chap:results} rests, so that these later claims can be
scrutinised against the exact input they consume.

Section~\ref{sec:cs_environment} introduces the HHLA Container Terminal
Burchardkai (CTB), the truck-handling scope of the study and the data
...
event-log characteristics that are used verbatim as input to the
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    The case study site is the HHLA Container Terminal Burchardkai
    ...
    \begin{itemize}
        \item 22 RMG storage blocks (T06--T27),
        \item two specialised areas: an empty-container depot (LL) and a
              heavy/oversized cargo area (HO2),
        \item two gate lanes (inbound and outbound).
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item 336 yard-state snapshots (8 weeks $\times$ 7 days $\times$
              6 snapshots per day).
    \end{itemize}
...
Process mining and simulation require an explicit representation of
activity lifecycle states. For each activity, three timestamps are
constructed:

\begin{itemize}
    \item \textbf{Enabled time} (\texttt{enabled:timestamp}) — the moment
          the activity became eligible for execution but had not yet
          started.
...
The source systems do not explicitly record all three lifecycle states,
so additional reconstruction is required.

\subsubsection{Lifecycle Concept in Terminal Operations}

In traditional process mining an activity becomes enabled once all
preconditions for execution are fulfilled, even if execution cannot
start immediately due to unavailable resources. In container-terminal
operations this concept translates to the moment when a truck has
physically reached the next processing location and is ready to be
handled. The distinction between enabled, start and complete
timestamps is particularly relevant because trucks frequently
experience congestion, traffic delays and resource-related waiting;
explicit lifecycle reconstruction lets us separate physical movement
from operational waiting and service execution.

\subsubsection{Transition Baseline Estimation}
\label{sec:transition_baseline}

Reconstructing enabled timestamps requires an estimate of the physical
travel time between consecutive operational locations. The scheduling
system records observed transition durations
(\texttt{FP\_DAUER\_GATEIN\_ERSTER\_ALP\_SEK},
\texttt{ALP\_DAUER\_NAECHSTER\_ALP\_SEK} and
\texttt{FP\_DAUER\_LETZT\_ALP\_GATEOUT\_SEK}), but these observations
already include the effects of traffic, congestion and operational
delays.

To derive a congestion-free reference duration, historical transition
times are extracted from the scheduling data and grouped by
$(\text{origin}, \text{destination})$. For each transition, the 5th
percentile of the empirical transition-time distribution is used as
the baseline and stored in \texttt{transition\_baseline.csv} (script
\texttt{preparation/010\_generate\_transition\_baseline.ipynb}).

The 5th percentile is a compromise between robustness and realism.
While the minimum observed duration may be influenced by measurement
errors, the median already contains substantial congestion. The lower
tail therefore provides a realistic approximation of travel time under
uncongested conditions. The sensitivity of this choice to alternative
percentiles is evaluated in Section~\ref{sec:results_percentile_sens}.

\subsubsection{Enabled Timestamp Reconstruction}

Enabled timestamps are reconstructed by combining observed lifecycle
events with the baseline transition durations. For all activities
following gate entry:
\begin{equation}
    \text{enabled}_i = \text{complete}_{i-1} + \text{baseline}_{i-1,i}.
\end{equation}
The resulting timestamp represents the moment at which the truck should
arrive at the next operational location under uncongested conditions.

For the \textbf{Gate In} activity, detailed gate information is
available from the report system. The OCR gate scans the truck before
it enters the gate-processing queue; the OCR timestamp is therefore
used as the enabled timestamp:
$\text{enabled}=\text{StageStartTime}$,
$\text{start}=\text{FP\_ZEITPUNKT\_ERSTELLUNG}$ and
$\text{complete}\approx\text{start}$.

For \textbf{yard activities}
$\text{enabled}=\text{previous\_complete}+\text{transition\_baseline}$,
$\text{start}=\text{ALP\_ZEITPUNKT\_BEREITMELDUNG}$ and
$\text{complete}=\text{ALP\_ZEITPUNKT\_ERFUELLUNG}$. The interval
between enabled and start captures additional delays caused by traffic
congestion, queue formation, handling-equipment availability and other
operational constraints.

For \textbf{Gate Out} the same reconstruction principle is applied:
$\text{enabled}=\text{last\_yard\_complete}+\text{baseline}$,
$\text{start}=\text{complete}=\text{FP\_ZEITPUNKT\_GATEOUT}$.

\subsubsection{Waiting Time Calculation}

For each activity waiting time is defined as
\begin{equation}
    \text{waiting\_time}=\text{start:timestamp}-\text{enabled:timestamp}.
\end{equation}
By construction the enabled timestamp already includes the expected
physical travel time between operational locations. The calculated
waiting time therefore represents time beyond normal truck movement and
can be interpreted as operational delay caused primarily by traffic
congestion, queueing, resource contention and limited handling
equipment.

\medskip

\noindent\textbf{Methodological caveat.} The 5th percentile is an
empirical choice; the reconstructed waiting time therefore inherits its
uncertainty. The sensitivity analysis in
Section~\ref{sec:results_percentile_sens} quantifies how strongly the
reconstructed baselines shift under the alternative percentile choices
$\{5, 10, 25, 50\}$ so that this design decision is explicit in the
final results.
...
\texttt{enabled:timestamp} & Reconstructed activity enabled timestamp. \\
\texttt{start:timestamp}   & Activity start timestamp. \\
\texttt{time:timestamp}    & Activity completion timestamp. \\
