...
This chapter describes the operational context and the pipeline that turns raw
records into a simulation-ready event log. It covers the data design but does
not fit models, evaluate results, or choose experimental parameters. The event
log is the input for Chapters~\ref{chp:experiments_and_framework}
and~\ref{chap:results}.

Section~\ref{sec:cs_environment} introduces HHLA Container Terminal
Burchardkai (CTB), the truck-handling scope, and the data sources.
Section~\ref{sec:event_log_engineering} describes how the pipeline reconstructs
lifecycle timestamps and derives the case-level context required by ProSiT
\cite{Vinci2026prosit}. The final section describes the event log used for the
train/test split in Section~\ref{sec:train_test_split}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    The case study concerns HHLA Container Terminal Burchardkai (CTB) in the
    ...
    process. A Terminal Operating System coordinates them. This study uses the
...
\texttt{s6\_eventlog\_target\_rank\_features.csv}. The frozen temporal
partitions are exported as \texttt{s6\_train.csv},
