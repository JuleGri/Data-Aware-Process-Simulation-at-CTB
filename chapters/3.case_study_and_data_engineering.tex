...
\hline
\texttt{FAHRPLAN\_UID} & Unique truck-visit identifier used as the case ID. \\
\texttt{LKW\_KENNZEICHEN} & Truck licence plate number. \\
\texttt{ANLAUFPUNKT\_HALTESTELLE} & Exchange-area identifier, such as T20,
LL or VC. \\
\texttt{FP\_ZEITPUNKT\_ERSTELLUNG} & Time at which the schedule entry was
created; used as the Gate In timestamp. \\
\texttt{ALP\_ZEITPUNKT\_BEREITMELDUNG} & Time at which the handling activity
was reported ready for execution. \\
\texttt{ALP\_ZEITPUNKT\_ERFUELLUNG} & Time at which the handling activity was
completed. \\
\texttt{FP\_ZEITPUNKT\_GATEOUT} & Time at which the truck exited the
terminal. \\
\hline
\end{tabular}
...
\textbf{Attribute} & \textbf{Description} \\
\hline
\texttt{TruckVisitGkey} & Truck-visit identifier used for cross-system
matching. \\
\texttt{TruckLicenseNbr} & Truck licence plate number. \\
\texttt{CallupTime} & Time at which the truck physically entered the
terminal. \\
\texttt{StageStartTime} & Start time recorded for the reported process
stage. \\
\texttt{StageId} & Stage identifier, such as \texttt{OCR},
\texttt{Gate-In}, \texttt{yard} or \texttt{Exit}. \\
\texttt{TruckExchangeArea} & Yard handling point visited for a specific
container. \\
\texttt{TranCtrNbr} & Container number associated with the transaction. \\
\texttt{UnitInboundCarrierId} & Inbound carrier identifier, recorded as a
truck plate, vessel voyage number or train ID. \\
\texttt{UnitOutboundCarrierId} & Outbound carrier identifier, recorded as a
truck plate, vessel voyage number or train ID. \\
\hline
\end{tabular}
...
Full/empty status & Binary loading-status indicator
...
\textbf{Attribute} & \textbf{Description} \\
\hline
\texttt{start:timestamp} & Observed or reconstructed time at which execution
of the activity began. \\
\texttt{time:timestamp} & Observed or reconstructed time at which execution
...
\hline
Gate In & \texttt{FP\_ZEITPUNKT\_ERSTELLUNG} & Same value as the start
timestamp. \\
Yard activity & \texttt{ALP\_ZEITPUNKT\_BEREITMELDUNG} &
\texttt{ALP\_ZEITPUNKT\_ERFUELLUNG} \\
Gate Out & \texttt{FP\_ZEITPUNKT\_GATEOUT} &
\texttt{FP\_ZEITPUNKT\_GATEOUT} \\
\hline
\end{tabular}
