...
control-flow encoding and its held-out coverage
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Control-Flow Encoding and Held-Out Coverage Protocol}
\label{sec:process_discovery_methodology}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Process Discovery}

\subsection{Raw Prefix-Trie Encoding}

The Petri net that ProSiT uses as the authoritative control-flow backbone is constructed from the observed training variants as a raw sequential prefix trie, following the definition in Chapter~\ref{chp:background}. This is not a classical process-discovery abstraction: it stores the observed activity
language and does not infer additional behaviour. Every accepted path has the form
...
useful when behavioural generalisation is the objective
...
contract. The diagnostic tree in
...
subtree. Executing this model could permit Gate Out before all
...
This choice is deliberately restrictive. A raw observed-variant trie does not
generalise to unseen activity combinations and is less compact than an
Inductive-Miner model. Its perfect construction-log fitness is guaranteed and
is not treated as a result. The held-out conformance protocol instead tests
whether the memorised training language remains representative in a later
period. High held-out coverage would be evidence of a repetitive and stable
CTB trace language, not of generalisation by the trie. Accordingly, the final
control-flow component is best described as a domain-constrained encoding of
observed variants. Automatic discovery remains responsible for the other
ProSiT parameters, but the trie itself does not support a claim that a general
CTB process structure was discovered.
...
\subsection{Held-Out Coverage and Contract Protocol}

Control-flow conformance metrics are computed against \emph{both} the training
and the held-out testing partition:
...
\begin{itemize}
    ...
    \item Token-based replay \textbf{precision}, the share of behaviour
          allowed by the model and observed in the log.
...
For this model class, training fitness and precision are construction checks.
Held-out fitting traces are interpreted as the share of later cases whose
complete activity sequence already occurred during training. Generalisation
and simplicity remain useful descriptors of the cost of memorisation, but are
not combined into an overall process-discovery quality claim.
...
\texttt{'empirical'} for reasons given in
